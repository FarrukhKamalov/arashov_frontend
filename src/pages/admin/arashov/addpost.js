...
const AddPost = () => {
    ...
    const onSubmitCourses = async (data) => {
        const obj = {
            postText: inputs.videoTitle,
            postImage: inputs.videoLink
        }

        if (id) {
            if (type === "studentsedit") {
                axios.patch(`https://arashovplatform.onrender.com/api/v1/posts/${id}`, payment).then(router.push("/admin/arashov/dashboard"))
            } else {
                axios.patch(`https://arashovplatform.onrender.com/api/v1/posts/${id}`, obj).then(router.push("/admin/arashov/dashboard")).then(() => {
                    axios.get(`https://arashovplatform.onrender.com/api/v1/posts/${id}`).then(res => setInputs(res.data.data))
                })
            }
        } else {
            axios.post("https://arashovplatform.onrender.com/api/v1/posts/add", obj).then(router.push("/admin/arashov/dashboard"))
        }
    };
    ...
    useEffect(() => {
        if (id) {
            if(type === "studentsedit") {
                setPostOrPut(false)
                axios.get(`https://arashovplatform.onrender.com/api/v1/posts/${id}`).then(res => {
                    setInputs({
                        videoLink: res.data.data.postImage,
                        videoTitle: res.data.data.postText,
                    })
                })
            } else {  
                setPostOrPut(false)
    ...
    return (
        ...
        <div className="absolute w-full min-h-full bg-[#151A20] z-50 top-0 right-0 flex items-start justify-center">
            ...
            <Head>
                <title>Arashov - {postOrPut ? "POST" : "PUT"} POSTS</title>
            </Head>
                        ...
                        <div className="w-full my-3 gap-6 py-3 rounded-2xl px-5">
                            ...
                            ?
                                <><div className="w-full">
                                    ...
                                    <textarea onInput={e => setInputs({...inputs, videoTitle: e.target.value})} value={inputs?.videoTitle} {...register("videoTitle")} className="outline-0 border-0 text-gray-200 bg-gray-700 rounded-md px-5 py-2 w-full focus:ring-2 ring-gray-600" type="text" placeholder="Post title..." name="videoTitle" id="" />
                                </div>
                                
                                <div className="w-full my-3 gap-6 py-3 rounded-2xl">
                                    <h1 className="text-xl">Post image URL</h1>
                                    <input onInput={e => setInputs({...inputs, videoLink: e.target.value})} value={inputs?.videoLink}  {...register("videoLink")} className="outline-0 border-0 bg-gray-700 text-gray-200 rounded-md px-5 py-2 w-full focus:ring-2 ring-gray-600" type="text" placeholder="Post image URL..." name="videoLink" id="" />
                                </div></>
                                :
                                <div className="w-full my-3 gap-6 py-3 rounded-2xl">
                                    <div className="my-5">
